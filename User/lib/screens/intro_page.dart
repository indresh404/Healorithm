...
const List<_FeatureSlide> _slides = [
  _FeatureSlide(
    ...
    headline: 'Patient QR\nHealth Profile',
    body:
    'Each patient receives a unique digital Health ID with a QR code. Health workers can instantly scan it to view name, age, phone number, allergies, chronic diseases, and medical history — preventing lost records and repeated tests.',
    icon: Icons.qr_code_rounded,
    lottieAsset: 'assets/animations/login_animation_1.json',
  ),
  _FeatureSlide(
    tag: 'OFFLINE FIRST',
    headline: 'Works Without\nInternet',
    body:
    'All patient data is stored locally using SQLite or Hive. Health workers can view records, update symptoms, log medicines, and add visit notes even without internet. Data automatically syncs to the cloud when connectivity returns.',
    icon: Icons.cloud_off_rounded,
    lottieAsset: 'assets/animations/login_animation_2.json',
  ),
  _FeatureSlide(
    tag: 'SMART TRIAGE',
    headline: 'Guided Symptom\nAssessment',
    body:
    'The system asks structured medical questions based on symptoms like fever, cough, chest pain, or vomiting. It then determines a risk level: Green for home care, Yellow for monitoring, and Red for urgent hospital referral.',
    icon: Icons.monitor_heart_rounded,
    lottieAsset: 'assets/animations/login_animation_3.json',
  ),
  _FeatureSlide(
    tag: 'PATIENT SEARCH',
    headline: 'Find Patients\nInstantly',
    body:
    'Health workers can quickly locate patients using QR scan, phone number, or name search. This enables fast access to patient records during home visits and medical checkups.',
    icon: Icons.search_rounded,
    lottieAsset: 'assets/animations/login_animation_4.json',
  ),
  _FeatureSlide(
    tag: 'HEALTH TIMELINE',
    headline: 'Complete Medical\nHistory',
    body:
    'Each patient has a detailed health timeline including previous symptoms, doctor visits, prescriptions, pregnancy tracking, vaccination history, and treatment records.',
    icon: Icons.timeline_rounded,
    lottieAsset: 'assets/animations/login_animation_5.json',
  ),
  _FeatureSlide(
    tag: 'MEDICINE TRACKING',
    headline: 'Smart Medicine\nReminders',
    body:
    'Health workers can add prescribed medicines and the system automatically generates dosage schedules, treatment duration, and reminders — all functioning even when offline.',
  ...
  _FeatureSlide(
    ...
    headline: 'Hospital\nReferral Support',
    body:
    'If a patient is marked high risk, the system recommends referral to nearby hospitals. Workers can record referral reasons, hospital details, and follow-up status for proper case tracking.',
...
class _SlideViewState extends State<_SlideView>
  ...
  void _setupAnimations() {
    ...
    _headFade = Tween<double>(begin: 0, end: 1).animate(
        CurvedAnimation(
            parent: _controller, curve: const Interval(0.15, 0.6)));
    _headSlide =
    ...
    _bodyFade = Tween<double>(begin: 0, end: 1).animate(
        CurvedAnimation(
            parent: _controller, curve: const Interval(0.3, 0.75)));
    _bodySlide =
    ...
    _imgFade = Tween<double>(begin: 0, end: 1).animate(
        CurvedAnimation(
            parent: _controller, curve: const Interval(0.2, 0.7)));
    ...
    return Column(
      ...
      children: [
        ...
        AnimatedBuilder(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(widget.slide.icon, color: Colors.white, size: 14),
        ...
        AnimatedBuilder(
          animation: _controller,
          builder: (_, child) =>
              FadeTransition(opacity: _bodyFade, child: child),
          child: Container(
            padding:
            ...
            Center(
              child: Lottie.asset(
                ...
                errorBuilder: (_, __, ___) =>
                    _FallbackIllustration(icon: widget.slide.icon),
              ),
            ),
          ],
        ),
...
class _ScrollHintButtonState extends State<_ScrollHintButton>
  ...
  void initState() {
    ...
    _bounce = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 900))
      ..repeat(reverse: true);
    _offset = Tween<double>(begin: 0, end: 8)
